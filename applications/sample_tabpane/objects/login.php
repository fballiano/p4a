...
class Login extends P4A_Mask
{
	function &Login()
	{
		...
		*/
		...
		$this->tab_pane->pages->build("p4a_frame", "frame");
		
		$this->tab_pane->pages->frame->setLabel("Label frame 1");
		
		$this->tab_pane->pages->frame->anchorCenter($this->message);
		$this->tab_pane->pages->frame->anchor($this->username);
		$this->tab_pane->pages->frame->anchor($this->password);
		$this->tab_pane->pages->frame->newRow();
		$this->tab_pane->pages->frame->anchorCenter($this->login);
		
		$this->tab_pane->setWidth(500);
		$this->tab_pane->setHeight(100);
		
		
		$this->build("p4a_field", "description");
		$this->description->setWidth(200);
		
		
		
		$this->tab_pane->pages->build("p4a_frame", "frame2");
		$this->tab_pane->pages->frame2->setLabel("Label frame 2");		
		$this->tab_pane->pages->frame2->anchor($this->description);
		$this->tab_pane->pages->frame2->setWidth(550);
		
		$this->tab_pane->pages->build("p4a_canvas", "canvas1");
		
		$this->tab_pane->pages->canvas1->anchor($this->description, 10);
		
		$this->tab_pane->pages->build("p4a_button", "bottone");
		
		
		
		
		
		$this->tab_pane->pages->build("p4a_tab_pane", "tab_pane_inside");
		$this->tab_pane->pages->tab_pane_inside->pages->build("p4a_button", "bottone_dentro_1");
		$this->tab_pane->pages->tab_pane_inside->pages->build("p4a_button", "bottone_dentro_2");
		$this->tab_pane->pages->tab_pane_inside->pages->build("p4a_button", "bottone");
		
		$this->tab_pane->pages->tab_pane_inside->setHeight(70);
		
		//$this->tab_pane->setActivePage($this->tab_pane->pages->frame2);
		$this->tab_pane->setActivePage("frame2");
		...
		*/
		$this->display("main", $this->tab_pane);
