...
class P4A_Filesystem_Navigator_Folders extends P4A_Widget
{
	...
	function hasFolders()
	{
		...
		while (false !== ($file = readdir($handle))) {
			if (is_dir("$base/$file") and ($file != ".") and ($file != "..") and ($file != "CVS") and ($file != ".svn") and ("$base/$file" != P4A_UPLOADS_TMP_DIR)) {
				return true;
			}
		}
		closedir($handle);
	...
	function getAsString($folder = "")
	{
		...
		foreach ($files as $file) {
			...
			if (is_dir($full_path) and ($file != ".") and ($file != "..") and ($file != "CVS") and ($file != ".svn") and ($full_path != P4A_UPLOADS_TMP_DIR)) {
