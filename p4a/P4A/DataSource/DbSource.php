...
class DbSource extends DataSource
{
    ...
    public function setWhere($where)
    {
        ...
        if ($this->_pointer !== null and !$this->isNew()) {
            $this->firstRow();
        }
        return $this;
    }
    ...
    public function getFilters()
    {
        ...
        foreach ($this->_filters as $string => $obj) {
            if (is_object($obj)) {
                ...
                if (isset($obj->data_field) and is_object($obj->data_field) and method_exists(
                        $obj->data_field,
                        'getNewValue'
                    )
                ) {
                    ...
                    trigger_error('Filters can be applied only to P4A\Widget\Field or P4A\DataField', E_USER_ERROR);
                }
    ...
    {
        if ($this->_is_loaded) {
            trigger_error('P4A\DataSource\DbSource::load() can be called only once', E_USER_ERROR);
        }
        ...
        foreach ($select->getPart('from') as $alias => $table_data) {
            $p4a_db_table = new DbTable(array(
        ...
        foreach ($select->getPart('columns') as $column_data) {
            ...
            if ($column_name == '*') {
                ...
                if (isset($this->_tables_metadata[$table_name]) and in_array(
                        $column_name,
                        array_keys($this->_tables_metadata[$table_name]['metadata'])
                    )
                ) {
    ...
    {
        $this->fields->build("P4A\DataField", $name);
    ...
    public function saveRow($fields_values = array(), $pk_values = array())
    {
        if (!$this->isReadOnly()) {
            ...
            $pks = $this->getPk();
            $p4a_db_table = new DbTable(array('name' => $table, 'schema' => $schema, 'db' => $db->adapter));
            if ($this->isNew()) {
                ...
                foreach ($this->_multivalue_fields as $fieldname => $aField) {
                    ...
                    if (!empty($toremove)) {
                        foreach ($toremove as $k => $v) {
                            $db->adapter->query(
                                "DELETE FROM $fk_table WHERE $fk=? AND $fk_field=?",
                                array($pk_value, $v)
                            );
                        }
                    }
                    ...
                    if (!empty($toadd)) {
                        foreach ($toadd as $k => $v) {
                            $db->adapter->query(
                                "INSERT INTO $fk_table($fk, $fk_field) VALUES(?, ?)",
                                array($pk_value, $v)
                            );
                        }
                    }
    ...
    protected function _composeSelectStructureQuery()
    {
        ...
        $select = Db::singleton($this->getDSN())->select();
        $this->_composeSelectPart($select);
        $this->_composeWherePart($select);
        $this->_composeGroupPart($select);
    ...
    protected function _composeSelectPkQuery($pk_value)
    {
        ...
        if (is_array($pk_key)) {
            ...
            $pk_value = Db::singleton($this->getDSN())->quote($pk_value, true);
    ...
    protected function _composePkString($pk_values = array())
    {
        ...
        if (is_numeric($pk_values) or is_string($pk_values)) {
            return "{$this->getPk()} = " . Db::singleton($this->getDSN())->quote($pk_values, true);
        } elseif (is_array($pk_values)) {
            $return = '';
            foreach ($pk_values as $key => $value) {
                $return .= "$key = " . Db::singleton($this->getDSN())->quote($value, true) . " AND ";
            }
    ...
    public function addMultivalueField($fieldname, $table = null, $fk = null, $fk_field = null)
    {
        ...
        if (!$fk_field) {
            $p4a_db_table = new DbTable(array('name' => $table, 'db' => $db->adapter));
        ...
        $this->fields->build("P4A\DataField", $fieldname);
