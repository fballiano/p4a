    ...
    private function _format($value, $type, $num_of_decimals)
    {
        ...
        switch ($type) {
            ...
            case 'datetime':
                ...
                return $date->get(\Zend_Date::DATES, $this->_locale_engine) . ' ' . $date->get(
            ...
            case 'integer':
                return \Zend_Locale_Format::toNumber(
                    $value,
                    array('precision' => 0, 'locale' => $this->_locale_engine)
                );
            case 'float':
                ...
                return \Zend_Locale_Format::toNumber(
                    $value,
                    array('precision' => $num_of_decimals, 'locale' => $this->_locale_engine)
                );
            case 'decimal':
                if ($num_of_decimals === null) {
                    $num_of_decimals = 2;
        ...
        switch ($type) {
            ...
            case 'date':
                $date = \Zend_Locale_Format::getDate(
                    $value,
                    array('locale' => $this->_locale_engine, 'fix_date' => true)
                );
            ...
            case 'time':
                ...
                $date = \Zend_Locale_Format::getDate(
                    $value,
                    array('date_format' => $date_format, 'locale' => $this->_locale_engine)
                );
                if (!isset($date['hour'])) {
                    $date['hour'] = '00';
                }
            ...
            case 'datetime':
                $date = \Zend_Locale_Format::getDateTime(
                    $value,
                    array('locale' => $this->_locale_engine, 'fix_date' => true)
                );
                $date['month'] = str_pad($date['month'], 2, 0, STR_PAD_LEFT);
            ...
            case 'float':
                ...
                return \Zend_Locale_Format::getFloat(
                    $value,
                    array('precision' => $num_of_decimals, 'locale' => $this->_locale_engine)
                );
            case 'decimal':
