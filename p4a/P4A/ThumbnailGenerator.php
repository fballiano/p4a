    ...
    public function processFile()
    {
        ...
        if ($this->thumbnail_width) {
            ...
            if ($this->thumbnail_max_height and ($this->thumbnail_height > $this->thumbnail_max_height)) {
                ...
                $this->thumbnail_width = round(
                    $this->thumbnail_height * $this->original_width / $this->original_height
                );
            }
        ...
        if ($this->isCacheEnabled()) {
            $this->cached_filename = $this->cached_filename_prefix . md5(
                    "{$this->filename}|{$size}|{$this->thumbnail_width}|{$this->thumbnail_height}"
                );
            switch ($this->filetype) {
                case IMAGETYPE_GIF:
