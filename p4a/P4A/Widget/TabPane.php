...
class TabPane extends Widget
{
    ...
    {
        $this->pages->build('P4A\Widget\Frame', $page_name);
        if ($label !== null) {
    ...
    {
        if ($this->actionHandler('beforeSetActivePage') == ABORT) {
            return ABORT;
        }
        ...
        switch (gettype($page)) {
            ...
            default:
                trigger_error(
                    'P4A_Tab_pane "' . $this->getName() . '": unable to set "' . gettype(
                        $page
                    ) . '" as active page, reason: unsopported type',
                    E_USER_ERROR
                );
                break;
        }

        if ($this->actionHandler('afterSetActivePage') == ABORT) {
            return ABORT;
        }
    ...
    public function getAsString()
    {
        ...
        while ($page = $this->pages->nextItem()) {
            ...
            $mouseover = "";
            if (strlen($tooltip)) {
                $tooltip = "<div id='{$page->getId(
                )}tooltip' class='p4a_tooltip'><div class='p4a_tooltip_inner'>$tooltip</div></div>";
        ...
        $return .= "</ul>";
        $return .= "<div class='p4a_tab_pane_page ui-tabs-panel ui-widget-content ui-corner-all ui-helper-clearfix' style='height:$height'>" . $this->getActivePage(
            )->getAsString() . "</div>";
        $return .= "</div>";
