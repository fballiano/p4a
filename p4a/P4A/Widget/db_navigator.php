...
class P4A_DB_Navigator extends P4A_Widget
{
     ...
     */
    public function setSource(P4A\DataSource\DbSource $source)
    {
        $this->source =& $source;
        return $this;
    }
    ...
    public function getAsString()
    {
        ...
        if (strlen($js) and $this->allow_movement_to_root) {
            ...
            if (strlen($root_label)) {
                $root_label = __($root_label);
            }
            if (strlen($root_label) == 0) {
                $root_label = "&nbsp;";
            }
    ...
    public function getPath($id, $table, $pk)
    {
        ...
        $section = P4A_DB::singleton($this->source->getDSN())->adapter->fetchRow(
    ...
    public function onClick($params)
    {
        ...
        if ($this->actionHandler('beforeClick', $params) == ABORT) {
            return ABORT;
        }
    ...
    public function onMovement()
    {
        ...
        if ($new_value != $current) {
            if (strlen($new_value)) {
                P4A_DB::singleton($this->source->getDSN())->adapter->query(
                    "UPDATE $table SET {$this->recursor} = ? WHERE $pk = ?",
                    array($new_value, $current)
                );
            } else {
                P4A_DB::singleton($this->source->getDSN())->adapter->query(
                    "UPDATE $table SET {$this->recursor} = NULL WHERE $pk = ?",
                    array($current)
                );
            }
        }
