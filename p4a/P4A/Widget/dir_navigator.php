...
class P4A_Dir_Navigator extends P4A_Widget
{
    ...
    private function _getAsString($base_dir)
    {
        ...
        foreach (scandir($base_dir) as $dir) {
            ...
            if (!is_dir($absolute_dir) or $absolute_dir == P4A_UPLOADS_TMP_DIR or substr(
                    $dir,
                    0,
                    1
                ) == '.' or $dir == 'CVS'
            ...
            if (P4A_OS == "linux") {
                $actions = $this->composeStringActions(
                    str_replace(
                        P4A_Strip_Double_Slashes("{$this->base_dir}/"),
                        "",
                        P4A_Strip_Double_Slashes($absolute_dir)
                    )
                );
            } else {
                $actions = $this->composeStringActions(
                    str_replace(
                        P4A_Strip_Double_Backslashes("{$this->base_dir}\\"),
                        "",
                        P4A_Strip_Double_Backslashes($absolute_dir)
                    )
                );
            }
