...
class Field extends Widget
{
    ...
    public function __construct($name, $add_default_data_field = true)
    {
        ...
        if ($add_default_data_field) {
            $this->build('P4A\DataField', 'data_field');
        }

        $this->build('P4A\Widget\Label', 'label');
    ...
    protected function getVisualizedDataType()
    {
        ...
        if (!is_null($source_description_field) and is_object($this->data)) {
            if (!isset($this->data->fields->$source_description_field)) {
                trigger_error("P4A\Widget\Field is missing: {$source_description_field}", E_USER_ERROR);
            }
    ...
    public function setNewValue($new_value)
    {
        ...
        if ($new_value === null) {
            $new_value = null;
        } elseif (($this->type == 'multicheckbox' or $this->type == 'multiselect') and $this->multivalue_separator and is_array(
                $new_value
            )
        ) {
    ...
    public function getNewValue($index = null)
    {
        ...
        if ($new_value === null) {
            // $new_value = null;
        } elseif (($this->type == 'multicheckbox' or $this->type == 'multiselect') and $this->multivalue_separator and is_string(
                $new_value
            )
        ) {
    ...
    public function setType($type, $multivalue_separator = null)
    {
        ...
        switch ($type) {
            ...
            case 'file':
                ...
                break;
            default:
                trigger_error("$type is not a supported P4A\Widget\Field type", E_USER_ERROR);
        }
        $this->type = $type;
        return $this;
    }
    ...
    protected function format($value, $type = null, $num_of_decimals = null)
    {
        ...
        if ($num_of_decimals === null) {
            $num_of_decimals = $this->data_field->getNumOfDecimals();
        }
    ...
    public function getAsString()
    {
        ...
        if ($visualized_data_type) {
            $css_classes[] = "p4a_field_data_$visualized_data_type";
        }
    ...
    public function getAsTime()
    {
        ...
        if (!P4A::singleton()->isHandheld()) {
            ...
            if ($enabled) {
                $close_header .= "<input type='hidden' value='$value' name='p4a_{$id}' id='p4a_{$id}' onchange=\"p4a_time_select('p4a_{$id}', '{$id}input')\" />";
            }
            $options = '{}';
        ...
        return $this->composeLabel() . $header . $this->composeStringProperties() . $this->composeStringValue(
    ...
    public function getAsDateTime()
    {
        ...
        if (!P4A::singleton()->isHandheld()) {
            $value = $this->data_field->getNewValue();
            if ($enabled) {
                $close_header .= "<input type='hidden' value='$value' name='p4a_{$id}' id='p4a_{$id}' onchange=\"p4a_datetime_select('p4a_{$id}', '{$id}input')\" />";
            }
            $options = '{';
            if ($this->_year_range) {
    ...
    public function getAsDate()
    {
        ...
        if (!P4A::singleton()->isHandheld()) {
            $value = $this->data_field->getNewValue();
            if ($enabled) {
                $close_header .= "<input type='hidden' value='$value' name='p4a_{$id}' id='p4a_{$id}' onchange=\"p4a_calendar_select('p4a_{$id}', '{$id}input')\" />";
            }
            $options = '{';
            if ($this->_year_range) {
                $options .= "yearRange:'{$this->_year_range}',";
    ...
    public function getAsTextarea()
    {
        ...
        $sReturn = $this->composeLabel() . "<div class='br'></div>{$header}" . $this->composeStringProperties(
            ) . $this->composeStringActions() . $close_header;
    ...
    public function getAsRichTextarea()
    {
        ...
        $this->addTempVar(
            "connector",
            urlencode(P4A_APPLICATION_PATH . "/index.php?_rte_file_manager=1&_object_id=" . $this->getId())
        );
    ...
    {
        $id = $this->getId();
        return "<input type='hidden' id='{$id}input' " . $this->composeStringProperties() . $this->composeStringValue(
    ...
    public function getAsLabel()
    {
        ...
        return $this->composeLabel() . $header . $this->composeStringProperties() . $this->composeStringActions(
        ) . $close_header . $value . $footer;
    }
    ...
    public function getAsSelect()
    {
        ...
        foreach ($external_data as $key => $current) {
            ...
            if ($this->isFormatted()) {
                $sContent .= htmlspecialchars(
                    $this->format(
                        $current[$description_field],
                        $this->data->fields->$description_field->getType(),
                        $this->data->fields->$description_field->getNumOfDecimals()
                    )
                );
            } else {
    ...
    public function getAsMulticheckbox()
    {
        ...
        foreach ($external_data as $key => $current) {
            ...
            $sReturn .= P4A_Generate_Widget_Layout_Table(
                "<input type='checkbox' id='{$id}_{$i}input' name='{$id}[]' value='{$current[$value_field]}' $checked $disabled />",
                "<label for='{$id}_{$i}input'>{$current[$description_field]}</label>"
            );
            $i++;
        }
    ...
    public function getAsRadio()
    {
        ...
        foreach ($external_data as $key => $current) {
            ...
            $input = "<input $enabled name='{$id}' id='{$id}_{$key}input' type='radio' " . $this->composeStringActions(
                ) . " $checked value='" . htmlspecialchars($current[$value_field]) . "'/>";
            if ($this->isFormatted()) {
                $label = $this->format(
                    $current[$description_field],
                    $this->data->fields->$description_field->getType(),
    ...
    public function getAsFile()
    {
        ...
        if ($this->getNewValue() === null) {
            ...
            $sReturn .= '<tr><th>' . __('Size') . ':</th><td>' . $p4a->i18n->format(
                    $this->getNewValue(2) / 1024,
                    "decimal"
                ) . ' KB</td></tr>';
            ...
            $sReturn .= '<tr><td colspan="2">' . $this->buttons->button_file_preview->getAsString(
                ) . ' ' . $this->buttons->button_file_download->getAsString(
                ) . ' ' . $this->buttons->button_file_delete->getAsString() . '</td></tr>';
            $sReturn .= '</table>';
        }
    ...
    public function getAsImage()
    {
        ...
        if (P4A_GD) {
            ...
            $sReturn .= '<tr><td colspan="2"><img alt="' . __('Preview') . '" src=".?_p4a_image_thumbnail=' . urlencode(
                    "$src&{$this->max_thumbnail_size}"
                ) . '" /></td></tr>';
        }
        ...
        $sReturn .= '<tr><th>' . __('Name') . ':</th><td>' . $name . '</td></tr>';
        $sReturn .= '<tr><th>' . __('Size') . ':</th><td>' . P4A::singleton()->i18n->format(
                $this->getNewValue(2) / 1024,
                "decimal"
            ) . ' KB</td></tr>';
        $sReturn .= '<tr><th>' . __('Type') . ':</th><td>' . $this->getNewValue(3) . '</td></tr>';
    ...
    public function addValidator(\Zend_Validate_Interface $validator, $break_chain_on_failure = false)
    {
        if ($this->_validator_chain === null) {
            $this->_validator_chain = new Validate();
        }
