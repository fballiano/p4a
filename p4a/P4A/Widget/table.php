...
class Table extends Widget
{
     ...
     * @return P4A_Table
     */
    public function setSource(DataSource $data_source)
    {
    ...
    {
        $this->build('P4A\Collection', 'cols');
    ...
    {
        $this->cols->build('P4A\Widget\TableCol', $column_name);
...
class TableCol extends Widget
{
     ...
     * @return unknown
     */
    public function onClick($aParams = null)
    {
        if ($this->isActionTriggered('beforeclick')) {
            if ($this->actionHandler('beforeclick', $aParams) == ABORT) {
                return ABORT;
            }
        }

        if ($this->isActionTriggered('onclick')) {
            if ($this->actionHandler('onclick', $aParams) == ABORT) {
                return ABORT;
            }
        } else {
            if ($this->getType() == 'action') {
                ...
                if ($table->data->row(
                        $aParams[0] + (($table->getCurrentPageNumber() - 1) * $table->data->getPageLimit()) + 1
                    ) == ABORT
                ) {
                    return ABORT;
                }
        ...
        if ($this->isActionTriggered('afterclick')) {
            if ($this->actionHandler('afterclick', $aParams) == ABORT) {
                return ABORT;
            }
        }
    }
...
class TableRows extends Widget
{
    ...
    public function getRows($num_page, $rows)
    {
        ...
        foreach ($rows as $row_number => $row) {
            ...
            foreach ($aCols as $col_name) {
                ...
                $aReturn[$i]['cells'][$j]['action'] = ($enabled and $row_enabled and $col_enabled) ? $this->composeStringActions(
                    array($row_number, $col_name)
                ) : '';
                ...
                if ($parent->cols->$col_name->data) {
                    ...
                    if ($row_enabled and $col_enabled) {
                        ...
                        $aReturn[$i]['cells'][$j]['action'] = $parent->cols->$col_name->composeStringActions(
                            array($row_number, $col_name)
                        );
                    } else {
                        $aReturn[$i]['cells'][$j]['action'] = $enabled ? $parent->cols->$col_name->composeStringActions(
                            array($row_number, $col_name)
                        ) : '';
                    }
                } else {
                    if ($parent->cols->$col_name->isFormatted()) {
                        if ($parent->cols->$col_name->isActionTriggered('onformat')) {
                            $aReturn[$i]['cells'][$j]['value'] = $parent->cols->$col_name->actionHandler(
                                'onformat',
                                $row[$col_name],
                                $parent->data->fields->$col_name->getType(),
                                $parent->data->fields->$col_name->getNumOfDecimals(),
                                $row
                            );
                        } else {
                            $aReturn[$i]['cells'][$j]['value'] = $p4a->i18n->format(
                                $row[$col_name],
                                $parent->data->fields->$col_name->getType(),
                                $parent->data->fields->$col_name->getNumOfDecimals(),
                                false
    ...
    public function onClick($aParams = null)
    {
        ...
        if ($this->isActionTriggered('onclick')) {
            ...
            if ($parent->data->row(
                    $aParams[0] + (($parent->getCurrentPageNumber() - 1) * $parent->data->getPageLimit()) + 1
                ) == ABORT
            ) {
                return ABORT;
            }
 ...
 * @package p4a
 */
class TableNavigationBar extends Frame
{
    /**
     * @var P4A_Collection
     */
    ...
    public function __construct()
    {
        ...
        $this->buttons->build('P4A\Widget\Box', 'current_page');
        $this->anchorLeft($this->buttons->current_page);

        $this->buttons->build("P4A\Widget\Box", "s1")
        ...
        $this->buttons->build("P4A\Widget\Box", "s2")
            ->addCSSClass("p4a_toolbar_separator")
            ->setHeight(self::height)
            ->setVisible(false);
    ...
    {
        $button = $this->buttons->build("P4A\Widget\Button", $button_name);
