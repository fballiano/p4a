    ...
    public function preChange($params = null)
    {
        ...
        if ($this->cols->$col_name->isFormatted()) {
            if ($this->cols->$col_name->isActionTriggered('normalize')) {
                $params[3] = $this->cols->$col_name->actionHandler(
                    'normalize',
                    $value,
                    $this->data->fields->$col_name->getType(),
                    $this->data->fields->$col_name->getNumOfDecimals()
                );
            } else {
                $params[3] = $p4a->i18n->normalize(
                    $value,
                    $this->data->fields->$col_name->getType(),
                    $this->data->fields->$col_name->getNumOfDecimals(),
                    false
    ...
    public function getRows($num_page, $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($aCols as $col_name) {
                if ($this->cols->$col_name->isEnabled() and ($this->cols->$col_name->getType(
                        ) == "action" or (isset($this->data->fields->$col_name) and !$this->data->fields->$col_name->isReadOnly(
                            )))
                ) {
                    $col_enabled = true;
                ...
                if ($this->cols->$col_name->getType() == "action") {
                    $aReturn[$i]['cells'][$j]['value'] = __($this->cols->$col_name->getLabel());
                    $aReturn[$i]['cells'][$j]['type'] = 'action';
                    if ($col_enabled) {
                        $aReturn[$i]['cells'][$j]['clickable'] = 'clickable';
                        $aReturn[$i]['cells'][$j]['action'] = $this->cols->$col_name->composeStringActions(
                            array($i, $col_name)
                        );
                    } else {
                        $aReturn[$i]['cells'][$j]['action'] = $enabled ? $this->cols->$col_name->composeStringActions(
                            array($i, $col_name)
                        ) : '';
                    }
                } elseif ($this->cols->$col_name->isFormatted()) {
                    if ($this->cols->$col_name->isActionTriggered('onformat')) {
                        $aReturn[$i]['cells'][$j]['value'] = $this->cols->$col_name->actionHandler(
                            'onformat',
                            $row[$col_name],
                            $this->data->fields->$col_name->getType(),
                            $this->data->fields->$col_name->getNumOfDecimals()
                ...
                if (!isset($aReturn[$i]['cells'][$j]['type'])) {
