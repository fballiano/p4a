...
class P4A extends Object
{
    ...
    public function timer($description = 'TIMER')
    {
        ...
        $this->timer[$num_record]['diff'] = $this->timer[$num_record - 1]['diff'] + (P4A_Get_Microtime(
                ) - $this->timer[$num_record - 1]['value']);
    }
        ...
        */
        ...
        if ($this->_redesign_whole_mask or $_REQUEST['_ajax'] == 2) {
            print '<ajax-response action_id="' . $this->getActionHistoryId(
                ) . '" focus_id="' . $this->getFocusedObjectId(true) . '">';
        } else {
            print '<ajax-response action_id="' . $this->getActionHistoryId(
                ) . '" focus_id="' . $this->getFocusedObjectId() . '">';
        }
    ...
    {
        $this->active_mask = Mask::singleton($mask_name);
    }
    ...
    {
        if ($this->actionHandler('beforeopenmask') == ABORT) {
            return ABORT;
        }

        if ($this->isActionTriggered('onopenmask')) {
            if ($this->actionHandler('onopenmask') == ABORT) {
                return ABORT;
            }
        } else {
            if ($this->inAjaxCall()) {
                ...
                foreach ($__P4A_REGISTERED_DB_CONNECTIONS as $dbconn) {
                    ...
                    if (is_array($queries) and !empty($queries)) {
                        foreach ($queries as $query) {
                            $this->messageInfo(
                                "DB: {$config["dbname"]}<br />Host: {$config["host"]}<br />{$query->getQuery(
                                )}<br />Time: {$query->getElapsedSecs()}"
                            );
                        }
                    }
                }
