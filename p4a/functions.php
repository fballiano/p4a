...
function __($string)
{
    return \P4A\P4A::singleton()->i18n->translate($string);
}
...
{
    if ($v === null) {
        $v = '<span style="color:green">NULL</span>';
    }
    if ($v === false) {
        $v = '<span style="color:green">FALSE</span>';
    }
    if ($v === true) {
        $v = '<span style="color:green">TRUE</span>';
    }
    if ($v === '') {
        $v = '<span style="color:green">Empty String</span>';
    }
    ...
    if (is_object($v)) {
        if (is_a($v, 'P4A\Object')) {
            $v = '<a href=".?_p4a_session_browser=' . $v->getId() . '">' . $v->getName() . ' (' . get_class(
                    $v
                ) . ')</a>';
        } else {
 ...
 * @return string
 */
function P4A_Generate_Widget_Layout_Table(
    $column1 = null,
    $column2 = null,
    $additional_css_classes = null,
...
function P4A_Error_Handler($error_number, $error_string, $error_file, $error_line)
{
    ...
    switch ($error_number) {
        ...
        case E_RECOVERABLE_ERROR:
            ...
            $error_mask = $p4a->openMask("P4A\Mask\Error")->setMessage($message);
            if ($p4a->inAjaxCall()) {
...
function p4a_check_configuration($additionalDir = null)
{
    ...
    if (defined('P4A_DSN')) {
        try {
            ...
            $error .= "<span class='red'>FAILED</span><br />Error: " . $e->getMessage(
                ) . "<br />Check P4A_DSN definition.";
            $correct = false;
        }
    } else {
