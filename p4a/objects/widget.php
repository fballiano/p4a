		...
		function composeStringActions($params = NULL)
		{
  			...
  			foreach ($this->actions as $action=>$action_data)
			{
				...
				if ($action_data['require_confirmation'])
				{
					...
					{
						$prefix .= 'if(confirm(\''. str_replace( '\'', '\\\'', $this->p4a->i18n->messages->get($action_data['confirmation_text_handler'])) .'\')){';
					}
					else
					{
				...
				$sActions .= $browser_action . '="' . $prefix . 'executeEvent(\'' . $this->id . '\', \'' . $action_data['event'] . '\''. $sParams .');' . $suffix . ' return ' . $return . ';" '; 
			}		
			return $sActions; 															
		}
