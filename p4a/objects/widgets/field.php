	...
	class P4A_FIELD extends P4A_WIDGET
	{
		...
		function format( $value )
		{
			if( strlen( $value ) > 0 )
			{
				...
				{
					$value = $this->p4a->i18n->{$this->formatter_name}->format( $value, $this->p4a->i18n->{$this->formatter_name}->getFormat( $this->format_name ) );
				}
				else
				{
					$value = $this->p4a->i18n->autoFormat( $value, $this->data_field->getType() );
				}
			}
		...
		function unformat( $value )
		{
			if( strlen( $value ) > 0 )
			{
				if( ( $this->formatter_name !== NULL ) and ( $this->format_name !== NULL ) )
				{
					$value = $this->p4a->i18n->{$this->formatter_name}->unformat( $value, $this->p4a->i18n->{$this->formatter_name}->getFormat( $this->format_name ) );
				}
				else
				{
					$value = $this->p4a->i18n->autoUnformat( $value, $this->data_field->getType() );
				}
			}
		...
		function getAsDate()
		{
			...
			$this->display('language', $this->p4a->i18n->getLanguage());
			$this->display('date_format', $this->p4a->i18n->datetime->getFormat('date_default'));
		...
		function getAsSelect()
		{
			...
			if( $this->isNullAllowed() )
			{
				if( $this->null_message === NULL ) {
					$message = $this->p4a->i18n->messages->get('none_selected');
				} else {
			...
			foreach( $external_data as $key=>$current )
			{
				...
				$sContent .= htmlspecialchars($this->p4a->i18n->autoFormat($current[ $description_field ], $this->data->structure[$description_field]['type']));
				$sContent .= "</option>";
		...
		function getAsRadio()
		{
			...
			foreach( $external_data as $key=>$current )
			{
				...
				unset( $sContent ) ;
				$sContent  = $this->p4a->i18n->autoFormat( $current[ $description_field ], $this->data->structure[$description_field]['type']);
