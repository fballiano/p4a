 ...
 */
	...
	class P4A_SHEETS_GROUP extends P4A_WIDGET
	{
		...
		function addSheet( &$sheet, $name, $position = NULL )
		{
			...
			//$this->labels[$sheet->name]->intercept($sheet, 'set_label', 'setValue');
			$this->intercept($sheet, 'set_label', 'setSheetLabel');
			$this->labels{$sheet->name}->addAction('onClick');
 			$this->intercept($this->labels{$sheet->name}, 'onClick', 'labelsOnClick');
		...
		function setActive( $sheet_name )
		{
			if( $this->active_sheet !== NULL )
			{
				if( $this->labels[ $this->active_sheet->name ] !== NULL )
				{
					$this->labels[ $this->active_sheet->name ]->enable();
		...
		{
			$this->labels[$sheet->name]->setValue($label);
		}
		...
		{
			$this->setActive($this->p4a->active_object->name);
		}
