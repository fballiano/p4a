	...
	public function getRows($num_page, $rows)
	{
		...
		foreach ($rows as $row) {
			...
			foreach($aCols as $col_name) {
				if ($this->cols->$col_name->isEnabled() and ($this->cols->$col_name->getType() == "action" or (isset($this->data->fields->$col_name) and !$this->data->fields->$col_name->isReadOnly()))) {
					$col_enabled = true;
				...
				if ($this->cols->$col_name->getType() == "action") {
					...
					if ($col_enabled) {
						...
						$aReturn[$i]['cells'][$j]['action'] = $this->cols->$col_name->composeStringActions(array($i, $col_name));
					} else {
						$aReturn[$i]['cells'][$j]['action'] = $enabled ? $this->cols->$col_name->composeStringActions(array($i, $col_name)) : '';
					}
